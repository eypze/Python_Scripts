...
def build_schema_from_csv(csv_file_path: str) -> tuple[str, list[bigquery.SchemaField]]:
    ...
    table_name = df["TABNAME"].iloc[0] if "TABNAME" in df.columns else ""
    if not table_name:
        raise ValueError("'TABNAME' column not found or empty in the CSV file.")
    ...
    for _, row in df.iterrows():
        ...
        field_type  = row.get("DATATYPE", "STRING") or "STRING"
        length      = str(row.get("LENG", "") or "")
        decimals    = str(row.get("DECIMALS", "") or "")
        description = str(row.get("DDTEXT", "") or "")
...
# ---------------------------------------------------------------------------
if __name__ == "__main__": # ejecute in terminal with "python gcp_tables_deploy.py"
